...
```{r}
ggplot() +
  geom_segment(data = highest_peak_finish, aes(x = Radical_distance, xend = Radical_distance,
                                               y = -1, yend = g_r),
               linetype = "dashed", color = "red", inherit.aes = FALSE) +
  geom_text(data = highest_peak_finish, aes(x = Radical_distance, y = g_r,
                                            label = sprintf("%.3f", Radical_distance)),
            vjust = 2.8, hjust = -0.2, color = "black", inherit.aes = FALSE)
...
ggplot(data_c_filtered_finish, aes(x = Radical_distance, y = g_r, color = Measurement)) +
  ...
  scale_color_manual(values = c("D_D2-Eu" = "black", "D_D2-H" = "red", 
                                 "D_D2-N" = "purple", "D_D2-C" = "green", 
                                 "D_D2-O" = "cyan"),
                     breaks = c("D_D2-Eu", "D_D2-H", "D_D2-N", "D_D2-C", "D_D2-O")) +
  ...
  geom_text(data = highest_peak_finish, aes(x = Radical_distance, y = g_r,
                                            label = sprintf("%.3f", Radical_distance)),
            vjust = 27, hjust = -0.2, color = "black", inherit.aes = FALSE) +
...
ggplot(data_c_filtered_finish, aes(x = Radical_distance, y = g_r, color = Measurement)) +
  ...
  scale_color_manual(values = c("D_D2-Eu" = "black", "D_D2-H" = "red", "D_D2-N" = "purple", "D_D2-C" = "green","D_D2-O" = "cyan"),
   breaks = c("D_D2-Eu", "D_D2-H", "D_D2-N",  "D_D2-C","D_D2-O")) +
  ...
  geom_label(data = highest_peak_finish, aes(x = Radical_distance * 0.1, y = g_r, label = sprintf("%.3f ", Radical_distance)),
             vjust = 15.5, hjust = -1.5, color = "blue", fill = "white", fontface = "bold", size = 3.5, label.size = 0.5) +
...
```{r}
data_d_long_new <- data_d %>%
  pivot_longer(
    cols = starts_with("..."),
    names_to = "region", 
    values_to = "od_percentage" 
  ) %>%
  mutate(
    od_percentage = case_when(
      od_percentage == "OD%<0.2Å" ~ 0.2,
      od_percentage == "OD%<0.1Å" ~ 0.1,
      TRUE ~ as.numeric(od_percentage)
    )
  )

data_d_long_new <- data_d_long_new %>%
  filter(!is.na(`CSD code`))
head(data_d_long_new)
```

```{r}
colnames(data_d_long)
```
...
```{r}
data_d_long <- clean_names(data_d_long)
colnames(data_d_long)
```
...
```{r}
str(data_d_long$od_percentage)

unique(data_d_long$od_percentage)

```

```{r}
data_d_long <- data_d_long %>%
  mutate(od_percentage = case_when(
    od_percentage == "OD%<0.2Å" ~ 0.2,
    od_percentage == "OD%<0.1Å" ~ 0.1,
    TRUE ~ as.numeric(as.character(od_percentage))
  ))
...
```{r}
data_d_long <- data_d_long %>%
  filter(!grepl("OD%", od_percentage)) %>%
  mutate(od_percentage = as.numeric(as.character(od_percentage)))
```

```{r}
str(data_d_long$od_percentage)
sum(is.na(data_d_long$od_percentage))
```
...
ggplot(data_d_long, aes(x = factor(csd_code, levels = unique(csd_code)), 
                        y = od_percentage, 
                        fill = region)) +
  geom_violin(trim = FALSE, alpha = 0.5, color = NA) + 
  geom_boxplot(width = 0.1, color = "gray", alpha = 0.8, outlier.shape = NA) + 
  geom_hline(yintercept = 50, linetype = "dashed", color = "red") + 
  scale_y_continuous(labels = scales::percent_format(scale = 1)) + 
  labs(x = "CSD Code", y = "Overlap Degree (%)", fill = "Region") + 
